...
function underTest() {
    ...
    if (esm) {
        ...
        const {HashMap, Mootable} = esmRequire(location);
        return {
            HashMap,
            Mootable,
            name, location, esm
        ...
        const {HashMap, Mootable} = require(location);
        return {
            HashMap,
            Mootable,
            name, location, esm
...
describe('hamt.hashmap', function () {
    let HashMap, Mootable;
    let hashmap;
    before(function () {
        const UnderTest = underTest();
        HashMap = UnderTest.HashMap;
    ...
    describe('constructor', function () {
        ...
        it('should clone a hashmap when one argument with copy', function () {
            ...
            let map = new HashMap({copy: hashmap, hamt:true});
            expect(map.length).to.equal(2);
            expect(map.get('key')).to.equal('value');
            expect(map.get('key2')).to.equal('value2');
        });
